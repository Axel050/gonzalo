                    ...
                    <div class="items-start lg:w-auto w-[85%] mx-auto" x-data="{ existe: false }">
                        <div class="flex w-full justify-between bg-ed-100 py-0 items-end">
                            ...
                            <label class="text-gray-500 bgred-100 py-0 leading-[16px]">
                                Existe
                                <input type="checkbox" x-model="existe" @click.stop wire:model="existe" />
                            </label>
                        </div>
                        <div class="relative w-full">
                            <template x-if="!existe">
                                <input
                                    class="lg:w-60 h-6 rounded-md border border-gray-400 w-full text-gray-500 p-2 text-sm bg-gray-100"
                                    wire:model="alias" />
                            </template>
                            <template x-if="existe">
                                <select
                                ...
                                @php
                                    $url =
                                        $foto && method_exists($foto, 'temporaryUrl')
                                            ? $foto->temporaryUrl()
                                            : ($foto
                                                ? Storage::url('imagenes/adquirentes/' . $foto)
                                                : asset('004b.jpg'));
                                @endphp
