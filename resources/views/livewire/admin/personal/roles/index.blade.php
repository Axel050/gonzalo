<div class="w-full  py-5">
    ...
    <x-action-message on="rolCreated" class="absolute  top-0 right-0 z-50 green-action ">Rol creado con
        exitó.</x-action-message>
    <x-action-message on="rolUpdated" class="absolute  top-0 right-0 z-50 orange-action">Rol actualizado con
        exitó.</x-action-message>
    <x-action-message on="rolDeleted" class="absolute  top-0 right-0 z-50 red-action">Rol eliminado con
        exitó.</x-action-message>

    <x-action-message on="permisosAdded" class="absolute  top-0 right-0 z-50 green-action">Permiso agregado con
        exitó.</x-action-message>
    ...
    <div class="">
        ...
        @if ($method && $method != 'permisos')
            @livewire('admin.personal.roles.modal', ['method' => $method, 'id' => $id])
        @endif

        @if ($method == 'permisos')
            @livewire('admin.personal.roles.permisos', ['id' => $id])
        @endif
                            ...
                            <tbody class="divide-y divide-gray-400 text-gray-600  text-sm bg-gray-300">
                                ...
                                @foreach ($roles as $rol)
                                    <tr
                                        ...
                                        <td class="py-2">{{ $rol->id }}</td>
                                        <td>{{ $rol->name }}</td>
                                        <td>{{ $rol->description }}</td>
                                        <td>{{ $rol->is_active ? 'Si' : 'No' }}</td>
                                        ...
                                        <td>
                                            <div class="flex justfy-end lg:gap-x-6 gap-x-4 text-white text-xs">
                                                ...
                                                <button
                                                    ...
                                                    wire:click="option('permisos',{{ $rol->id }})">
                                                    <svg width="20px" height="20px">
                                                        <use xlink:href="#permisos"></use>
                                                    </svg>
                                                ...
                                                <button
                                                    ...
                                                    wire:click="option('delete',{{ $rol->id }})">
                                                    <svg width="20px" height="20px">
                                                ...
                                                <button
                                                    ...
                                                    wire:click="option('update',{{ $rol->id }})">
                                                    <svg width="20px" height="20px">
