...
class SubastaService
{
    ...
    public function getLotesProximosaaa(
        ...
        return $query
            ->paginate(2)
            ->through(function ($lote) use ($conCaracteristicas) {
    ...
    {
        $subastas = Subasta::where('fecha_inicio', '>', Carbon::now())->where('estado', SubastaEstados::INACTIVA)->get();

        $lotes = $this->lotesPorSubasta(
            $subastas->pluck('id'),
            fn ($q) => $q->where('lotes.estado', LotesEstados::ASIGNADO)
                ->where('contrato_lotes.estado', 'activo')
                ->where('lotes.destacado', true)
        );
